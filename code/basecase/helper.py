...
def make_prediction_dummy(model, scaler):
    ...
    y_test_pred = np.exp(model.predict(Xd_test))
...
def make_prediction_le(model, scaler):
    ...
    y_test_pred = np.exp(model.predict(X_le_test))
...
def permutation_importances(rf, X_train, y_train, metric):
    ...
    # this returns negative values when using oob_regression_mse_score (adapted from oob_regression_r2_score)
