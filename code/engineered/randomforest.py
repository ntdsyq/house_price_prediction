...
from helper import plot_cv_traintestscores, make_prediction_le, make_prediction_le_noscaling, \
                    load_traindata, plot_FI_tree, display_tree, plot_cv_testscores
...
def tune_Nestimator(X,y):
    ...
    rf_model = ensemble.RandomForestRegressor(max_depth = 14, min_samples_split=3, max_features = 16, min_samples_leaf = 1)
    n_estimators = np.linspace(50, 400, 8, endpoint=True, dtype = int)
...
def tune_maxdepth(X,y):
    ...
    """
    # takes 23s to run
    rf_model = ensemble.RandomForestRegressor(n_estimators = 260)
    max_depths = np.linspace(10,20,11, endpoint=True, dtype=int)
    ...
    plot_cv_testscores(np.sqrt(-tuneRF.cv_results_['mean_test_score']), max_depths, 'max tree depth')
    print("best RMSE: ",np.sqrt(-tuneRF.best_score_))
    print("best max_depth: ",tuneRF.best_params_)
    print(tuneRF.best_estimator_)
    return tuneRF.best_estimator_
...
def tune_min_samples_leaf(X,y):
    ...
    """
    # takes 23s to run
    rf_model = ensemble.RandomForestRegressor(n_estimators = 250, max_depth = 14, min_samples_split=3, max_features = 16)
...
def tune_min_samples_split(X,y):
    ...
    """
    # takes 23s to run
    rf_model = ensemble.RandomForestRegressor(n_estimators = 250, max_depth = 14, max_features = 16)
...
def tune_max_features(X,y):
    ...
    """
    # takes 23s to run
    rf_model = ensemble.RandomForestRegressor(n_estimators = 250, max_depth = 14, min_samples_leaf = 1, min_samples_split = 2)
    max_features = np.linspace(10, 20, 10, endpoint=True, dtype = int)
    print(max_features)
...
def tune_Nestimators_maxdepth(X,y):
    ...
    nn = 21
    nm = 10
    n_estimators = np.linspace(100, 300, nn, endpoint=True, dtype = int)
    max_depths = np.linspace(2,20,nm, endpoint=True, dtype=int)
    grid_para = [{'n_estimators': n_estimators, 'max_depth': max_depths}]    
    tuneRF = GridSearchCV(rf_model, grid_para, cv=inner_cv, refit=True, return_train_score = True, scoring = 'neg_mean_squared_error', n_jobs = -1)
    t1 = time.time()
    tuneRF.fit(X,y)
    t2 = time.time()
    print(t2 - t1, " seconds\n")
    ...
    # each row correspond to one max_depth value
    test_rmse = np.sqrt(-tuneRF.cv_results_['mean_test_score']).reshape((nm,nn))
...
RF4 = tune_min_samples_split(X,y)  # n = 100 or 250 trees, 3 runs each, gives 3 as optimal 
RF3 = tune_min_samples_leaf(X,y)   # n = 100 or 250 trees, 3 runs each gives 1 as optimal

# make predictions with best model, most complex model, simplest model
best_RF1 = ensemble.RandomForestRegressor(n_estimators = 250, max_depth = 14, max_features = 16, min_samples_split = 3, min_samples_leaf = 1)
best_RF1.fit(X,y)
outdata1 =  make_prediction_le(best_RF1, scaler)
outdata1.to_csv("yq_submission34_RF_tuned.csv",index = False)
