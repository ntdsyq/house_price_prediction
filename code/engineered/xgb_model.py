...
def xgbmodelfit(alg, datamatrix, cvfolds ):
    ...
    plot_cv_traintestscores(cvresult['train-rmse-mean'][40:50], cvresult['test-rmse-mean'][40:50], [i for i in range(n)][40:50], 'n_estimators')
...
imp.to_csv("xgb_FI_submit"+str(submit_num)+".csv",header=True)
submissiondata = make_prediction_le_noscaling(xgb1)
submissiondata.to_csv("yq_submission"+str(submit_num) + "_xgb_start.csv",index = False)
...
print('Best mean test RMSE: %.5f' %(np.sqrt(-gs1.best_score_)) )
...
print('Best mean test RMSE: %.5f' %(np.sqrt(-gs3.best_score_)) )
...
print('Best mean test RMSE: %.5f' %(np.sqrt(-gs4.best_score_)) )
...
print('Best mean test RMSE: %.5f' %(np.sqrt(-gs5.best_score_)) )
xgb1.set_params(subsample = 0.55, colsample_bytree = 0.65)
...
print('Best mean test RMSE: %.5f' %(np.sqrt(-gs6.best_score_)) )
...
print('Best mean test RMSE: %.5f' %(np.sqrt(-gs7.best_score_)) )

param_test7_2 = {
 'reg_lambda': [0.85, 0.9, 0.95]
}

gs7_2 = GridSearchCV(xgb1, param_grid = param_test7_2, scoring='neg_mean_squared_error',n_jobs= -1, iid=False, cv= outer_cv)
gs7_2.fit(X,y)
print_grid_scores(gs7_2)
print('Best parameters: %r' %gs7_2.best_params_)
print('Best mean test RMSE: %.5f' %(np.sqrt(-gs7_2.best_score_)) )
xgb1.set_params(reg_lambda = 0.9)

# step 8: reduce learning rate, re-tune n_estimators
xgb1.set_params(learning_rate = 0.01, n_estimators = 3000)
xgbmodelfit(xgb1, xgtrain, outer_cv)
...
imp.to_csv("xgb_FI_submit"+str(submit_num)+".csv",header=True)
submissiondata = make_prediction_le_noscaling(xgb1)
submissiondata.to_csv("yq_submission"+str(submit_num) + "_xgb_tuned.csv",index = False)
