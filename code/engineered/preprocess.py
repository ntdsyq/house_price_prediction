...
def new_features_train(df):
    ...
    df['Remod_Rcy'] = df['YearRemodAdd'] - df['YearBuilt']  # recency of remod, the larger the number, the better
...
def make_train_data():
    ...
    df1['LogSalePrice'] = np.log1p(df1['SalePrice'])
    ...
    # additional outliers, improved CV score but not Kaggle score
    outliers2 = [30, 88, 462, 631, 1322]
    df1.drop(outliers2, axis = 0, inplace = True)
    df1.reset_index(drop=True, inplace=True)
    y = df1['LogSalePrice']
...
import pickle
with open('train_prep.pickle', 'wb') as f:
    pickle.dump([X_train, Xd_train, y_train, X_le_train], f)
    
with open('test_prep.pickle','wb') as f:
