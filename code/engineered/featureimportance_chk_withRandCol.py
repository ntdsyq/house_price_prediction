...
from helper import load_traindata, permutation_importances, oob_regression_mse_score
...
def FI_RF_permutation_blog():
    X, y = load_traindata(encodetype ='le')
    rf = ensemble.RandomForestRegressor(n_estimators = 250, max_depth = 14, max_features = 16, min_samples_split = 3, random_state = 42, oob_score=True)
...
def FI_xgb_blog():    
    ...
    xgb1 = XGBRegressor( 
            learning_rate=0.01, n_estimators=1996,
...
rf = ensemble.RandomForestRegressor(n_estimators = 250, max_depth = 14, max_features = 16, random_state = 1, oob_score=True)
rf_fi1 = FI_RF_sklearn(rf)
rf_fi1 = rf_fi1.reset_index()
rf_fi1.rename(columns={'index':'rank1','Importance':'Imp1'}, inplace=True)

rf_fi2 = FI_RF_permuation(rf)
...
rf_fi_compare = rf_fi_compare[['Feature','rank2','rank1','Imp2','Imp1']]
rf_fi_compare.to_csv("RF_FI_submit34.csv", index = False)
...
plt.tight_layout()
plt.savefig("../../documentation/blog/FI-after FS and FE.pdf")
